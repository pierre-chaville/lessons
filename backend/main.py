...
@app.get("/lessons/{lesson_id}/pdf/sources/detailed", tags=["Lessons"])
def get_lesson_detailed_sources_pdf(lesson_id: int, session: Session = Depends(get_session)):
    """Generate and download detailed PDF of all sources with full information (like the modal)"""
    from fastapi.responses import Response
    ...
    filename = f"{safe_title}_sources_detailed.pdf"
