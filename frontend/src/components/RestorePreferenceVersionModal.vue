...
const props = defineProps<{
  isOpen: boolean
  targetVersion: PreferenceVersion | null
  currentVersion: PreferenceVersion | null
}>()

const emit = defineEmits<{
  (e: 'close'): void
  (e: 'restored', version: PreferenceVersion): void
}>()
...
const onConfirm = async () => {
  ...
  error.value = null
  try {
    const restored = await configApi.restoreVersion(props.targetVersion.id, reason.value.trim())
...
<template>
  <Dialog :open="isOpen" @close="emit('close')" class="relative z-50">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
      <DialogPanel class="flex w-full max-w-3xl max-h-[90vh] flex-col overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-800">
