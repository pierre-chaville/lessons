...
import RestorePreferenceVersionModal from '@/components/RestorePreferenceVersionModal.vue'
...
const emit = defineEmits<{
  ...
  (e: 'restored'): void
  (e: 'versions-loaded', versions: PreferenceVersion[]): void
  (
    e: 'compare',
    payload: {
      versionAId: string
...
const canRestore = computed(() => can('configuration', 'update'))
...
const restoreTarget = ref<PreferenceVersion | null>(null)
...
const loadVersions = async (reset = false) => {
  ...
  try {
    ...
    const rows = await configApi.listVersions({ limit: 20, before })
    if (reset) {
      versions.value = rows
    } else {
...
const ensureSelectedVersion = () => {
  ...
  if (!exists) emit('update:selectedVersionId', null)
}
...
const sourceBadge = (source: PreferenceVersion['version_source']) => {
  if (source === 'human') return t('history.sourceHuman')
  if (source === 'restore') return t('history.sourceRestore')
  return t('preferences.sourceSystem')
}
...
const sessionDurationMinutes = (version: PreferenceVersion): number => {
  ...
  return Math.max(0, Math.ceil((endedAt - startedAt) / 60000))
}

const versionLabel = (version: PreferenceVersion): string => {
  if (version.version_source === 'system') {
    return t('history.versionLabelPipeline', { createdAt: formatDateTime(version.created_at) })
  }
  if (version.version_source === 'restore') {
    return t('history.versionLabelRestore', {
      ...
      author: getUserName(version.created_by_id),
    })
  }
