...
const parseUnifiedDiff = (rawDiff: string): DiffRow[] => {
  ...
  for (const line of rawDiff.split('\n')) {
    ...
    rows.push({ kind: 'context', oldLine, newLine, text: normalized })
    oldLine += 1
    newLine += 1
  }
...
const rows = computed(() => {
  if (!diff.value || diff.value.type !== 'text' || !diff.value.diff) return []
...
const fetchDiff = async () => {
  ...
  error.value = null
  try {
    diff.value = await configApi.getVersionDiff(props.versionAId, props.versionBId)
  } catch (e: any) {
...
watch(
  () => [props.versionAId, props.versionBId],
  fetchDiff,
  { immediate: true },
)
</script>
...
<template>
  <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800">
    ...
    <div v-else class="max-h-[65vh] overflow-auto rounded-md border border-gray-200 dark:border-gray-700">
      <div class="sticky top-0 z-10 grid grid-cols-[2rem_3.25rem_3.25rem_1fr] border-b border-gray-200 bg-gray-100 text-[11px] font-semibold uppercase tracking-wide text-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
        ...
        <div class="px-2 py-1">preferences.yaml</div>
      </div>
      <div
      ...
      >
        ...
        <div class="whitespace-pre-wrap px-2 py-0.5" v-html="escapeHtml(row.text)"></div>
      </div>
    </div>
  </div>
</template>
