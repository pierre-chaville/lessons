...
const loadConfig = async () => {
  try {
    ...
    for (const section of ['correction', 'edition', 'sources'] as const) {
...
const addPrompt = (section: 'correction' | 'edition' | 'sources') => {
  const cfg = config.value[section]
  if (!cfg.prompts) cfg.prompts = []
  cfg.prompts.push({ name: '', text: '' })
}

const removePrompt = (section: 'correction' | 'edition' | 'sources', index: number) => {
  const cfg = config.value[section]
